...
difference() {
    ...
    translate([20,35,2])
    cube([x-40,y-60,z]);
    translate([largetoken+5,largetoken-1,z-largetokenh])
    cylinder(largetokenh+1,largetoken,largetoken);
    for ( xbase = [largetoken+40,largetoken+75,largetoken+110] ) {
      translate([xbase,largetoken-1,z-largetokenh])
      cylinder(largetokenh,largetoken,largetoken);
    }
    for ( ybase = [smalltoken+30,smalltoken+52] ) {
      translate([smalltoken-0.5,ybase,z-smalltokenh])
      cylinder(smalltokenh,smalltoken,smalltoken);
    }
    translate([smalltoken-0.5,smalltoken+74,z-smalltoken2h])
    cylinder(smalltokenh,smalltoken,smalltoken);
    translate([smalltoken-0.5,smalltoken+96,z-smalltoken2h+7])
    cylinder(smalltokenh,smalltoken,smalltoken);
    translate([smalltoken-0.5,smalltoken+118,z-smalltoken2h+11])
    cylinder(smalltokenh,smalltoken,smalltoken);
    translate([x-8.5,smalltoken+125,z-smalltoken2h+8])
    cylinder(smalltokenh,smalltoken,smalltoken);
    for ( ybase = [smalltoken+100,smalltoken+75,smalltoken+50] ) {
      translate([x-8.5,ybase,z-smalltoken3h])
      cylinder(smalltokenh,smalltoken,smalltoken);
    }
    for ( xbase = [smalltoken+20,smalltoken+45,smalltoken+70,smalltoken+95] ) {
      translate([xbase,y-8.5,z-smalltoken4h])
